...
- 已有 HTTP server。
- 已有 `/api/health`。
- 已有随服务启动的内置 Web 控制台，根路径 `/` 和 `/ui` 可直接查看当前能力清单、数据概况、近期任务和下载目录监听批次；控制台数据来自 `/api/dashboard`，并已接入媒体库/下载目录快速创建、配置列表展示、AV 番号解析、JavDB/JavBus live 数据源切换搜索、首个候选一键验证、候选详情拉取验证、候选显式保存、已保存候选加载和选择锁定。
- 已有 `/api/config`。
...
- 已新增 live scraper 验证入口 `/api/scrapers`、`/api/scrapers/tmdb/search`、`/api/scrapers/tmdb/fetch`、`/api/scrapers/av/parse`、`/api/scrapers/av/search`、`/api/scrapers/av/fetch`、`/api/scrapers/av/verify` 与 `POST /api/scrapers/{provider}/candidates`；TMDB 通过 `TMDB_API_KEY` 和可选 `TMDB_BASE_URL` 配置，电影/电视剧搜索与详情获取默认只验证远端数据和字段映射，不写入候选表。AV 解析已支持标准番号、FC2、HEYZO、CARIB/1PONDO/10MUSUME 的归一化和推荐源路由，`source=auto` 会跳过未实现推荐源并按当前已接入的 JavDB/JavBus 顺序做 live 验证，前一个源无候选或候选详情均失败时继续尝试下一个源，且返回每个源和候选的搜索/详情状态；`candidate_limit` 默认验证前 3 个候选，可在 1-10 之间调整。JavDB/JavBus search/fetch/verify 已统一返回标准番号和封面 URL，便于后续评分、海报下载和媒体图片索引；验证通过的 live candidate 可显式保存到 `scrape_candidates`，复用现有评分和匹配状态刷新逻辑。
...
- 已有 live candidate 显式保存入口，会把用户确认过的 live 候选写入 `scrape_candidates`，并复用自动评分与 `match_status` 刷新；如果控制台已先拉取详情，保存时会用详情页 metadata 补强标题、年份、封面、简介，并把候选与详情原文写入 `raw_payload` 便于后续回放和排错。
