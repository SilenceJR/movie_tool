...
- 失败媒体文件可通过 `POST /api/media-files/{id}/retry` 单个重试，也可通过 `POST /api/media-files/retry-failed?library_id=...` 按媒体库批量重试；重试成功会清除 `failed` 状态、失败原因和失败时间。
