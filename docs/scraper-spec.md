...
`av/search` 与 `av/fetch` 已接入第一版 JavDB HTML 源解析，`JAVDB_BASE_URL` 默认 `https://javdb.com`；该实现用于验证搜索页/详情页可获取与字段映射，默认不写入数据库。
...
2. live search/fetch 验证：JavDB 已有第一版 search/fetch，先返回候选、封面、发行日期、简介，不写库。
