...
GET /api/download-directories/watch/runs?status={status}&limit={n}
...
`GET /api/download-directories/watch/runs` 返回最近的 `download_watch` 任务历史，可按 `status` 过滤并用 `limit` 限制数量；详细目录摘要可继续读取对应任务日志。
