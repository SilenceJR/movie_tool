...
`av/verify` 会执行“解析番号 -> 自动/指定源搜索 -> 拉取首个候选详情”的验证闭环，返回 `parsed`、`candidate`、`metadata` 与 `verified`，默认不写入数据库；`source=auto` 会按已实现推荐源顺序尝试，前一个源无候选时继续尝试下一个源，并在 `source_selection.attempted_sources` 中返回尝试路径，在 `source_selection.attempts` 中返回每个源的 `verified`、`no_candidates`、`search_failed` 或 `fetch_failed` 状态。
